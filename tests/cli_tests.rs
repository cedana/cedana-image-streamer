...
/// Helper to run cedana-image-streamer with --help and check it succeeds
fn get_help_output() -> String {
    let output = Command::new(env!("CARGO_BIN_EXE_cedana-image-streamer"))
        .arg("--help")
        .output()
...
fn test_short_option_images_dir() {
    ...
    let output_long = Command::new(env!("CARGO_BIN_EXE_cedana-image-streamer"))
        .args(["--images-dir", "/tmp", "capture", "--help"])
        .output()
        .expect("Failed to execute criu-image-streamer");

    let output_short = Command::new(env!("CARGO_BIN_EXE_cedana-image-streamer"))
