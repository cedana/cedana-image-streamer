...
integration-test: target/$(BUILD)/cedana-image-streamer
	bats --jobs 10 tests/integration.bats

test-junit: target/$(BUILD)/cedana-image-streamer
...
coverage:
	...
	llvm-profdata merge -sparse $(COVERAGE_PATH)/integration-*.profraw \
		-o target/llvm-cov-target/cedana_image_streamer-integration.profdata || true
